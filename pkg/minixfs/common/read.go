...
func ReadMap(inode *CacheInode, position int, cache BlockCache) int {
	...
	if excess < nr_indirects {
		...
		z = int(inode.Inode.Zone[dzones])
	} else {
		// 'position' can be located via the double indirect block
		z = int(inode.Inode.Zone[dzones+1])
		if z == NO_ZONE {
			return NO_BLOCK
		}
		excess = excess - nr_indirects // single indirect doesn't count
		b := z << scale
		bp := cache.GetBlock(inode.Devno, int(b), INDIRECT_BLOCK, NORMAL) // get double indirect block
	...
	bp := cache.GetBlock(inode.Devno, int(b), INDIRECT_BLOCK, NORMAL)
